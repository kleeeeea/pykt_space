# 来源：pykt-team/pykt-toolkit@2fc4d64880956f21155a238d0b0302ac42cb5196 pykt/models/dimkt.py
# PyPI 上的 pykt-toolkit 0.0.38 没有 DIMKT，所以原样复制过来；唯一改动见下方 "[本地改动]"
...
class DIMKT(Module):
    ...
    def forward(self,q,c,sd,qd,a,qshft,cshft,sdshft,qdshft):
        ...
        # [本地改动] 原文是 .cuda()，CPU-only 机器上会报错；改成 .to(self.device)，GPU 上行为不变
        k = self.knowledge.repeat(self.batch_size,1).to(self.device)
