...
class MoETransformerLayer(nn.Module):
    def __init__(self, d_model, d_feature, d_ff, n_heads, dropout, kq_same,
        ...
        if single_expert_type:
            ...
            # [本地改动] 原文的专家字典还有第 4 个 'mamba': MambaExpert(...)，它 import mamba_ssm，
            # 而 mamba_ssm 需要编译 CUDA 扩展，CPU-only 机器上装不了。这里去掉 mamba 专家，
            # 保留 cnn / lstm / attention 三个，路由(gate)和加权机制不变（gate 的输出维度会自动变成 3）。
            # 有 GPU 且装了 mamba_ssm 时，把这一项加回来即可恢复官方 4 专家版本。
