...
from pykt_gh.utils import debug_print  # [本地改动] pykt -> pykt_gh
...
def get_outputs(self, emb_qc_future_shift, que_h, data, add_name="", model_type='question'):
    outputs = {}

    if model_type == 'question':
        h_next = torch.cat([emb_qc_future_shift, que_h], axis=-1)
        y_question_next = torch.sigmoid(self.out_question_next(h_next))
        y_question_all = torch.sigmoid(self.out_question_all(que_h))
        ...
        #all predict
        y_concept_all = torch.sigmoid(self.out_concept_all(que_h))
...
class QIKTNet(nn.Module):
    ...
    def forward(self, q, c ,r,data=None):
        ...
        emb_qc_future_shift = emb_qc[:,1:,:]
        emb_qca_current = emb_qca[:,:-1,:]
        # question model
        que_h = self.dropout_layer(self.que_lstm_layer(emb_qca_current)[0])
        que_outputs = get_outputs(self,emb_qc_future_shift,que_h,data,add_name="",model_type="question")
        outputs = que_outputs
        ...
        concept_outputs = get_outputs(self,emb_qc_future_shift,concept_h,data,add_name="",model_type="concept")
