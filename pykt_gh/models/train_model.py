...
from pykt_gh.config import que_type_models  # [本地改动] pykt -> pykt_gh
import pandas as pd
...
def model_forward(model, data, rel=None):
    ...
    if model_name in ["atdkt"]:
        ...
        # [本地改动] 原文 `y, y2, y3 = model(dcur, train=True)`；sparseKT.forward(train=True) 在这个 commit 上
        # 返回 4 个值 (preds, y2, y3, c_reg_loss)，其中 c_reg_loss 恒为 0，会 "too many values to unpack"。
        # 取前 3 个；其它返回 3 个值的模型不受影响
        y, y2, y3 = model(dcur, train=True)[:3]
        ys = [y[:,1:], y2, y3]
    elif model_name in ["rekt"]:
